...
export default function App() {
  ...
  function itemToDb(item){ return {
    id:item.id, user_id:user.id, category:item.category, subcategory:item.subcategory,
    name:item.name, favorite:item.favorite,
    color:item.color, size:item.size, material:item.material,
    condition:item.condition, location:item.location,
    status:item.status, action_needed:item.actionNeeded,
    style:item.style, style_season:item.styleSeason, occasion:item.occasion,
    when_bought:item.whenBought, price:item.price,
    custom_sell_link:item.customSellLink, custom_donate_link:item.customDonateLink,
              ...
              <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                {STATUS_OPTIONS.map(s=>(
                  <button key={s.value} className="btn" onClick={()=>updateStatus(item.id,s.value)}
                    style={{padding:"7px 16px",fontSize:12,fontWeight:600,background:item.status===s.value?s.color:"rgba(255,255,255,.7)",color:item.status===s.value?"#fff":G.muted,border:`1.5px solid ${item.status===s.value?s.color:G.border}`}}>
                    {s.icon} {s.label}
                  </button>
                ))}
              </div>
